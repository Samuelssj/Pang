...
public class TileMap {
	...
	private BufferedImage mapa;// = new BufferedImage(larguraTela, AlturaTela, BufferedImage.TYPE_4BYTE_ABGR);
	...
	public TileMap(int mapaLargura, int mapaAltura, int tileLargura, int tileAltura, String nomeTileSet, String nomeMapaMatriz) {
		...
		try {
			...
			System.out.println("N�o conseguiu ler o Tileset");
			e.printStackTrace();
		}
	...
	public void montarMapa() {
		...
		for (int i = 0; i < mapaLargura; i++) {
			for (int j = 0; j < mapaAltura; j++) {
				tile = (camada[i][j] != 0) ? (camada[i][j] - 1) : 2809;
